...
export default function ARDashboard() {
  ...
  const [arRemarks, setArRemarks] = useState("");
  const [arSignature, setArSignature] = useState("");
  const [processingId, setProcessingId] = useState("");
  const [loadingId, setLoadingId] = useState("");

  const fetchBookings = () => {
    fetch("/api/bookings?status=JA_APPROVED_PENDING_AR")
      .then(res => res.json())
  ...
  const handleAction = async (id: string, action: "APPROVE" | "REJECT") => {
    if (!arSignature.trim()) return alert("Please provide your digital signature");
    setLoadingId(id);
    await fetch(`/api/bookings/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        status: action === "APPROVE" ? "AR_APPROVED_PENDING_CW" : "AR_REJECTED",
        arRemarks, arSignature, arApprovedAt: new Date().toISOString()
      }),
    });
    setLoadingId(""); setProcessingId(""); setArSignature(""); setArRemarks("");
    fetchBookings();
  };

  if (loading) return <div className="container" style={{ textAlign: "center", marginTop: "4rem" }}>Loading AR Dashboard...</div>;

  return (
    <div className="container" style={{ paddingBottom: '4rem' }}>
      <header style={{ marginBottom: '2rem' }}>
        <h2 style={{ textTransform: 'uppercase' }}>ASSISTANT REGISTRAR DASHBOARD</h2>
          ...
          {bookings.map(b => (
            <div key={b.id} className="glass-panel" style={{ padding: '2rem', display: 'grid', gridTemplateColumns: 'minmax(0, 1.5fr) minmax(0, 1fr)', gap: '2rem' }}>
              <div>
                ...
                {b.jaRemarks && <p><strong>JA Remarks:</strong> {b.jaRemarks}</p>}
              ...
              <div style={{ padding: '1.5rem', border: '2px solid black' }}>
                ...
                <div className="form-group">
                  ...
                  <input type="text" className="form-input" placeholder="Type name to sign" value={processingId === b.id ? arSignature : ""} onChange={(e) => { setProcessingId(b.id); setArSignature(e.target.value) }} />
                </div>
                <div className="form-group">
                  <label className="form-label">Remarks / Rejection Reason</label>
                  <textarea className="form-textarea" style={{ minHeight: '60px' }} value={processingId === b.id ? arRemarks : ""} onChange={(e) => { setProcessingId(b.id); setArRemarks(e.target.value) }} />
                </div>
